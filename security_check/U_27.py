...
def U27() :
    f_output = ""
    f_output = f_output + "[" + C_NUM + "] 불필요한 RPC 서비스 검사" + "\n"
    flag = False

    list = "rpc.cmsd rpc.ttdbserverd sadmind rusersd walld sprayd rstatd rpc.nisd rexd rpc.pcnfsd rpc.statd rpc.ypupdated rpc.rquotad kcms_server cachefsd"
    list = list.split()
    ...
    if (os.path.isfile(path)) :
        ...
        for i in list: # check
            if (data.find(i.upper()) != -1):
                f_output = f_output + C_YELLOW + "\t[경고] " + path + " : RPC 서비스(" + i + ")가 허용되어 있습니다.\n" + C_END
                flag = True
    ...
    for i in list: 
        if (os.path.isfile(path + i)):
            ...
            if (data.find("DISABLE=YES") == -1): # check
                f_output = f_output + C_YELLOW + "\t[경고] " + path + i + " : RPC 서비스가 허용되어 있습니다.\n" + C_END
                flag = True
